...
test('contributor PR workflow validates without auto-merging', () => {
  ...
  const installIndex = scanWorkflow.indexOf('npm ci --ignore-scripts');
  ...
  assert.match(scanWorkflow, /actions\/checkout@[0-9a-f]{40}/);
  assert.match(scanWorkflow, /actions\/setup-node@[0-9a-f]{40}/);
