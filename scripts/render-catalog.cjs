...
function renderStorefrontReadme(stores, index, auditJson) {
  ...
  lines.push('');
  lines.push('Core is the self-activating baseline every Alex ACT installation needs. Native Copilot CLI commands provide plugin lifecycle, and optional Alex ACT plugins add focused capabilities without a separate lifecycle controller.');
  lines.push('');
  ...
  lines.push('3. Add an optional plugin directly if you know what you need:');
