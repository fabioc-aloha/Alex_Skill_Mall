...
function renderStorefrontReadme(stores, index, auditJson) {
  ...
  lines.push('');
  lines.push('First-party plugins (🏆) rank highest because they earn the +50 provenance bonus. Heirs can override with `--from-store <name>` for any third-party alternative.');
  lines.push('');
  lines.push('---');
