...
function renderStorefrontReadme(stores, index, auditJson) {
  ...
  lines.push('');
  lines.push('# Recommended lifecycle install: Manager, then Core');
  lines.push('copilot plugin install alex-act-manager@alex-mall');
  ...
  lines.push('');
  lines.push('1. Install Manager with `copilot plugin install alex-act-manager@alex-mall`, then install Core with `copilot plugin install alex-act-core@alex-mall`.');
  lines.push('2. Reload VS Code, open Copilot Chat, and run `/alex-act-manager install-constellation`. This is the preferred lifecycle command; `/alex-act-core install-constellation` remains available for compatibility.');
