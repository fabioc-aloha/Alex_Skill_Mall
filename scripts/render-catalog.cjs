...
function renderStorefrontReadme(stores, index, auditJson) {
  ...
  lines.push('');
  lines.push(`**${curatedCount} curated plugins**, installable as a GitHub Copilot CLI marketplace. Plus a **trust-scored discovery index** across **${index.plugin_count} plugins** in **${index.store_count} stores** so you can find and install directly from upstream at a version you pick.`);
  lines.push('');
  ...
  lines.push('copilot plugin browse --marketplace alex-mall');
  lines.push('');
  lines.push('# Install a plugin by name');
  lines.push('copilot plugin install <plugin-name> --marketplace alex-mall');
  lines.push('');
  lines.push('# Example: install the visualization plugin');
  lines.push('copilot plugin install flint-chart-plugin --marketplace alex-mall');
  lines.push('```');
  lines.push('');
  ...
  lines.push('copilot plugin uninstall <name>        # remove a plugin');
  ...
  lines.push('2. **Install plugins via the Copilot CLI** using the steps above. Copilot Chat picks up any plugin components (agents, skills, commands, MCP servers) that ship with it.');
  ...
  lines.push('');
  lines.push('To make a project always pull specific plugins on first Chat session, commit a `.github/copilot/settings.json`:');
  lines.push('');
  lines.push('```jsonc');
  lines.push('{');
  lines.push('  "enabledPlugins": {');
  lines.push('    "flint-chart-plugin@alex-mall": true,');
  lines.push('    "document-banner-pastel@alex-mall": true');
  lines.push('  }');
  lines.push('}');
  lines.push('```');
  lines.push('');
  lines.push('On first session in that repo, Copilot Chat auto-registers `alex-mall` and installs the enabled plugins. This works for cloud-run sessions too.');
  lines.push('');
  lines.push('---');
  lines.push('');
  ...
  lines.push('- [By category](catalog/categories/) — 21 categories');
  ...
  lines.push('- [Source registry](sources/SOURCES.md) — the 49 upstream stores the discovery catalog aggregates');
  lines.push('');
