...
6. Read exact public versions from `alex-mall`'s `.github/plugin/marketplace.json` through `plugin-management/scripts/manager-operations.cjs marketplace-versions`. Use `marketplace browse` for discovery only, not version conclusions.
...
Fires only when the user explicitly asks — `/alex-act-manager plugin-status` is not auto-invoked. The audit is safe on any workspace, online or off.
