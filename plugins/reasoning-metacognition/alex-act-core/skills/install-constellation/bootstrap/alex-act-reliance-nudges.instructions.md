...
- [`emotional-intelligence.instructions.md`](emotional-intelligence.instructions.md) — same read surface (user messages) but reads *feeling state* rather than *epistemic behavior*. Both fire on every user turn; each catches a different failure mode.
