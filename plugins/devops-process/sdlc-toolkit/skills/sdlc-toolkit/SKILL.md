...
6. For ADRs, use the `ADR template reference`: Context → Decision → Consequences → Alternatives
...
2. Apply rules from the `validation rules reference`
...
2. Analyze historical patterns using the `risk scoring reference`
...
1. Use `code-search` MCP to scan source files for PII patterns from the `PII patterns reference`
...
1. Use `code-search` MCP to scan for debt markers (TODO, FIXME, HACK, XXX, WORKAROUND) across the repo using the `debt patterns reference`
