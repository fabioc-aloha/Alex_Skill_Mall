 ...
 *   --strip-frontmatter   Remove YAML frontmatter before conversion (default: enabled)
 *   --no-strip-frontmatter  Keep YAML frontmatter in the rendered document
 *   --strip-decorative-rules  Strip `---` thematic breaks (default: disabled — preserves semantic HRs)
 *   --no-strip-decorative-rules  Preserve `---` thematic breaks as visible <hr> in Word (default)
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    } else if (args[i] === '--strip-decorative-rules') {
      result.stripDecorativeRules = true;
    ...
    } else if (args[i] === '--no-strip-frontmatter') {
      result.stripFrontmatter = false;
